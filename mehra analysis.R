...
strength <- custom.strength(mehra.bootsamples, names(mehra.daily))

dag.mehra.99 <- averaged.network(strength, names(mehra.daily), 0.99)
...
graphviz.plot(subgraph(dag.mehra.99, names(mehra.daily)[-c(24, 26)]))
dev.off()

pdf("MEHRA_dag95.pdf", width = 15, height = 15)
graphviz.plot(subgraph(dag.mehra.95, names(mehra.daily)[-c(24, 26)]))
dev.off()

mehra.fit <- bn.fit(dag.mehra.99, mehra.daily)
mehra.fit[[15]]
